...
// --- New Relay Settings (Cycle Relay) ---
#define RELAY_CYCLE_PIN 32        // البن الخاص بالريلاي الجديد
// تم تحديث التوقيت ليتطابق مع دورة المضخة الأصلية (30 دقيقة تشغيل / 3.5 ساعة إيقاف)
unsigned long relayOnTime = 30UL * 60UL * 1000UL;  // 30 دقيقة
unsigned long relayOffTime = (4UL * 60UL * 60UL * 1000UL) - relayOnTime; // 3 ساعات و 30 دقيقة
unsigned long lastRelaySwitch = 0; // متغير لحفظ الوقت
bool relayCycleState = true;      // حالة الريلاي الحالية (يبدأ تشغيل)
// ----------------------------------------
...
const unsigned long runDuration = 30UL * 60UL * 1000UL; 
const unsigned long totalCycleDuration = 4UL * 60UL * 60UL * 1000UL;
...
const char *serverUrl = "http://plctemp.glitch.me/api/temperature";
...
// --- New Function: Handle Cycle Relay Logic ---
// هذا المنطق الآن يتبع دورة 30 دقيقة تشغيل / 3.5 ساعة إيقاف
...
  digitalWrite(RELAY_CYCLE_PIN, HIGH); // Start on
...
// دالة automaticControl() بقيت هنا ولكنها لم تعد تستدعى في loop()
// لأن منطق التوقيت الخاص بها تم دمجه في handleRelayCycle().
void automaticControl() {
  if (!automaticMode) return;
  unsigned long currentTime = millis();
  unsigned long elapsedTime = currentTime - lastCycleStart;
...
  } 
  else if (elapsedTime >= totalCycleDuration) {
...
  jsonDoc["totalFlow1"] = flowSensors[0].totalFlow; // تم التعديل لاستخدام struct مباشرة
  jsonDoc["flowSensorConnected1"] = flowSensors[0].flowSensorConnected; // تم التعديل لاستخدام struct مباشرة
  jsonDoc["flowRate2"] = flowSensors[1].flowRate; // تم التعديل لاستخدام struct مباشرة
  jsonDoc["totalFlow2"] = flowSensors[1].totalFlow; // تم التعديل لاستخدام struct مباشرة
  jsonDoc["flowSensorConnected2"] = flowSensors[1].flowSensorConnected; // تم التعديل لاستخدام struct مباشرة
  
...
  
  // Pump status is now tied to relay state in automatic mode
  jsonDoc["pumpRunning"] = digitalRead(PUMP_PIN);
  jsonDoc["pumpSpeed"] = digitalRead(PUMP_PIN) ? 100 : 0;
  
  // --- Add New Relay Status to JSON ---
...
  }
  ...
  // 1. تحديث حالة الريلاي (هو الآن من يحدد توقيت الـ 4 ساعات)
  handleRelayCycle();

  // 2. التحكم في المضخة والصمامات بناءً على الأوضاع
  if (automaticMode) {
    // الوضع التلقائي: المضخة والصمامات تأخذ حالتها من حالة الريلاي الدوري
    ...
    // الوضع اليدوي: المضخة والصمامات تتبع الحالة اليدوية
...
    automaticMode = false; // تعطيل التلقائي عند التحكم اليدوي
  } 
...
    automaticMode = true;
    lastCycleStart = millis(); // يتم الاحتفاظ بهذا المتغير لبدء الدورة الزمنية
  }
